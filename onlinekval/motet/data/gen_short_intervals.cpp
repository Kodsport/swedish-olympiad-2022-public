...
vector<vector<pair<ll, ll>>> short_intervals(ll n, ll min_m, ll max_m, ll sum_m, ll max_coordinate)
{
    ...
    rep(i, 0, n)
    {
        ...
        rep(j, 0, m)
        {
            ...
            if (vec[i].back().first > vec[i].back().second)
                swap(vec[i].back().first, vec[i].back().first);
            vec[i].back().second = vec[i].back().first + rand() % max((ll)1, (max_coordinate + 1 - vec[i].back().first) / 20);
        }
    }
    while (sum_m--)
    {
...
int main(int argc, char **argv)
{
    ...
    // Intervals with short intervals are generataed because I have noticed that when allowing intervals to be arbitrarily long
    // then many people have intervals that cover so many coordinates, which lead to the answer often being n
    ...
    vector<vector<pair<ll, ll>>> vec = short_intervals(n, min_m, max_m, sum_m, max_coordinate);
    cout << n << endl;
    trav(v, vec)
    {
        cout << v.size() << " ";
